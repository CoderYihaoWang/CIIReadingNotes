static char rcsid[] = "$Id: seq.c 6 2007-01-22 00:45:22Z drhanson $";
#include <stdlib.h>
#include <stdarg.h>
#include <string.h>
#include "assert.h"
...
static void expand(T seq) {
	...
	Array_resize(&seq->array, 2*n); /// copies the old array to a new larger array, the old is freed
	if (seq->head > 0)
		{
			void **old = &((void **)seq->array.array)[seq->head];
			/// there is no overlapping as n is larger than the length of the array, which is at most n
...
T Seq_new(int hint) {
	T seq;
	assert(hint >= 0);
	NEW0(seq); /// NEW0 includes initialisation to 0
	if (hint == 0)
		hint = 16;
...
T Seq_seq(void *x, ...) {
	...
	T seq = Seq_new(0);
	va_start(ap, x);
	for ( ; x; x = va_arg(ap, void *)) /// note that va_arg is implemented as a macro, which enables type to be a 'param'
		Seq_addhi(seq, x);
	va_end(ap);
	return seq;
}
void Seq_free(T *seq) {
	...
	Array_free((Array_T *)seq); /// ?? what is the size being freed ?? seems that free() frees the whole memory allocated
}
