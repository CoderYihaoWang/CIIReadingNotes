static char rcsid[] = "$Id: ring.c 6 2007-01-22 00:45:22Z drhanson $";
#include <stdlib.h>
#include <stdarg.h>
#include <string.h>
#include "assert.h"
...
/// a ring which contains 5 elements:
/// 1 2 3 4 5 6 <- positive position
/// |_|_|_|_|_|
/// |0|1|3|3|4| <- elements
/// |-|-|-|-|-|
///-5-4-3-2-1 0 <- non-positive position
...
void *Ring_get(T ring, int i) {
	...
	assert(i >= 0 && i < ring->length);
	{
		int n;
		q = ring->head;
		/// access from the nearest end
		if (i <= ring->length/2)
			for (n = i; n-- > 0; )
				q = q->rlink;
		else
			/// needs one additional move to move to the end of the ring
...
void *Ring_addlo(T ring, void *x) {
	assert(ring);
	/// add high and then move the head pointer
	Ring_addhi(ring, x);
...
void Ring_rotate(T ring, int n) { /// n must be valid positions
	struct node *q;
	int i;
	assert(ring);
	assert(n >= -ring->length && n <= ring->length);
	if (n >= 0)
		i = n%ring->length; /// -> if n == ring->length do nothing
	else
		i = n + ring->length;
	{
		int n;
