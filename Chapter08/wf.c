...
void wf(char *name, FILE *fp) {
	...
	while (getword(fp, buf, sizeof buf, first, rest)) {
		const char *word;
		int i, *count; /// count is to be put into the table, so it must be a pointer
		for (i = 0; buf[i] != '\0'; i++)
			buf[i] = tolower(buf[i]);
...
int compare(const void *x, const void *y) {
	/// x and y are pointers to pointers to chars when passed in
	/// they are converted to void * as args
	/// first recover them into char **
	/// then dereference them to get a char * as required by strcmp
