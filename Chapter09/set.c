...
struct T {
	...
	struct member {
		struct member *link;
		const void *member;
		/// however, there are only keys, no values
	} **buckets;
};
...
/// copy a set and possibly change the size
	...
	{ int i;
	  ...
	  for (i = 0; i < t->size; i++)
	  	for (q = t->buckets[i]; q; q = q->link)
		{
			...
			int i = (*set->hash)(member)%set->size; /// inner i: stands for hash value
			NEW(p);
			p->member = member;
			p->link = set->buckets[i];
			set->buckets[i] = p; /// this i is the inner i
			set->length++;
		} /// inner i is destroyed here, now ++ the outer i
	}
	return set;
}
T Set_new(int hint,
...
void *Set_remove(T set, const void *member) {
	int i;
	struct member **pp; /// using double pointer instead of single pointer
	assert(set);
	assert(member);
	set->timestamp++;
	i = (*set->hash)(member)%set->size;
...
/// toArray is the only means in which one can traverse a set
...
/// two operand sets must use the same compare and hush functions, unless one is empty
T Set_union(T s, T t) {
	if (s == NULL) {
		assert(t);
		...
		assert(s->cmp == t->cmp && s->hash == t->hash); /// ?? is this necessary ??
		{ int i;
		  struct member *q;
		  for (i = 0; i < t->size; i++)
		  	for (q = t->buckets[i]; q; q = q->link)
...
T Set_minus(T t, T s) {
	if (t == NULL){
		...
		return copy(t, t->size); /// different from inter
	else {
		T set = Set_new(Arith_min(s->size, t->size),
			s->cmp, s->hash);
		assert(s->cmp == t->cmp && s->hash == t->hash);
		{ int i;
		  ...
		  for (i = 0; i < t->size; i++)
		  	for (q = t->buckets[i]; q; q = q->link)
			if (!Set_member(s, q->member)) /// different from inter
				{
					struct member *p;
					const void *member = q->member;
					int i = (*set->hash)(member)%set->size;
...
T Set_diff(T s, T t) { /// diff = (A - B) + (B - A)
	if (s == NULL) {
		assert(t);
		return copy(t, t->size);
	} else if (t == NULL)
