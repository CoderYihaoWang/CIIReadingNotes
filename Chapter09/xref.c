static char rcsid[] = "$Id: xref.c 6 2007-01-22 00:45:22Z drhanson $";
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
#include "table.h"
...
int main(int argc, char *argv[]) {
	int i;
	/// global table to keep identifiers across files
	/// key: identifiers, value: a table where file names are the keys and sets of line numbers as values
	...
	/// output
	{
		...
		/// treat key, value together as a chunk to sort
		qsort(array, Table_length(identifiers),
			2*sizeof (*array), compare);
		for (i = 0; array[i]; i += 2) {
			/// array[i]: pointer to char
			/// array[i + 1]: pointer to a set which keeps line numbers and uses file names as keys
			printf("%s", (char *)array[i]);
			print(array[i+1]);
		}
		FREE(array); /// array returned by toArray must be freed manually, the elements in sets are not freed by this
	}
	return EXIT_SUCCESS;
}
int compare(const void *x, const void *y) {
...
void print(Table_T files) {
	...
	for (i = 0; array[i]; i += 2) {
		...
		{
			int j;
			void **lines = Set_toArray(array[i+1], NULL); /// lines: a pointer a an array of line numbers
...
void xref(const char *name, FILE *fp,
	...
	while (getword(fp, buf, sizeof buf, first, rest)) {
		...
		{
			/// set requires the items to pointers
			int *p = &linenum;
			if (!Set_member(set, p)) {
				NEW(p);
				*p = linenum;
